...
export const Polygon = styled.polygon`
	...
	fill: ${({ $isHovered, $isSelected, $isPrediction, $isPrefill, fill, index }) =>
		`${$isPrediction && !$isPrefill ? `url(#pattern_${index})` : $isHovered || $isSelected ? fill : 'transparent'}`};
	fill-opacity: ${({ $isHovered, $isSelected, $isPrediction }) => {
		if ($isHovered) {
			return 0.3
		} else if ($isPrediction || $isSelected) {
			return 0.5
		} else {
			return 1
		}
...
export const MarkerHeaderContainer = styled.foreignObject`
	...
	height: 20px;
	z-index: 999;
	opacity: ${({ $isHovered, $isSelected }) => `${$isHovered ? 0.8 : $isSelected ? 1 : 0}`};
`
