...
const NerContainer = ({
	...
	const resolvedContent = useMemo(() => {
		if (!isEmpty(highlights)) {
			...
			const resolvedMarksValue = (index) =>
			...
			const isHighlighted = (index) =>
				!isEmpty(highlights) && highlights.some(({ start, end }) => start <= index && index < end)
	...
	useEffect(() => {
		if (contentRef.current) {
			return setContentDimensions(contentRef.current.getBoundingClientRect())
		}
	}, [rootDimensions])
	...
	useEffect(() => {
		if (!isEmpty(contentRef.current) && !isEmpty(resolvedAnnotations)) {
			...
			const annotationsNodeProperties = annotationsNode.map((node) => node.getBoundingClientRect())

			setAnnotationsNodeProperties(annotationsNodeProperties)
		}
		...
		if (
			...
			if (!(startChar === ' ' && endChar === ' ')) {
				...
				if (!isEmpty(resolvedAnnotationsAndPredictions) || !isEmpty(highlights)) {
					...
					startIndex = startIndex + resolveNbrOfCharBefore(startParentNode)
					endIndex = endIndex + resolveNbrOfCharBefore(endParentNode)
				}
				...
				if (startIndex >= 0 && endIndex >= 0 && startIndex <= endIndex) {
					...
					if (!isEmpty(overlap)) {
						...
						const start =
							overlap.find(({ ner: { start, end } }) => findAnnotation(start, end, startIndex))?.ner.start || startIndex
						const end =
							overlap.find(({ ner: { start, end } }) => findAnnotation(start, end, endIndex))?.ner.end || endIndex
	...
	const _onMouseClick = useCallback(
		(index) => () => {
			if (isNumber(index) && onEntitiesRelationChange && selectedRelation) {
				if (!sourceRelation) {
					return setSourceRelation(resolvedAnnotations[index])
				}
