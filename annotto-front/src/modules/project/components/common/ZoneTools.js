...
const ZoneTools = ({
	...
	const _onSelectionChange = useCallback(
		(selectionValue, tasks) => (e) =>
			onSelectionChange && onSelectionChange(tasks.find(({ value }) => value === (e.target.value || selectionValue))),
		[onSelectionChange]
	)

	const _onRelationChange = useCallback(
		(selectionValue, entitiesRelationsGroup) => (e) =>
			onRelationChange &&
			onRelationChange(entitiesRelationsGroup.find(({ value }) => value === (e.target.value || selectionValue))),
		[onRelationChange]
	)
