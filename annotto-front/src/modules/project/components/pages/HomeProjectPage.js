...
import { ITEMS, TASKS } from 'shared/enums/projectStatsTypes'
...
const HomeProjectPage = ({ setHeaderActions }) => {
  ...
  const resolveTasksByCategories = useMemo(
    () =>
      groupTasksByCategories(tasks).map(({ category, children }, index) => ({
        key: index,
        value: category,
        children: children.map((child, keyIndex) => ({ key: `category_child_${keyIndex}`, ...child })),
      })),
    [tasks]
  )
