...
describe('annotationServices', () => {
	...
	describe('updateItemsService', () => {
		...
		it('Returns items if items is not an array', () => {
			const items = null
			expect(updateItemsService(items, itemToUpdate)).toEqual(items)
		})
		...
		it('Returns items with itemToUpdate in last position if itemToUpdate is not in items', () => {
			const itemToUpdate = { _id: 'zog', body: 'zog' }
			const expected = [...items, itemToUpdate]
		...
		it('Returns an array with itemToUpdate inside if items is an empty array', () => {
			const items = []
			expect(updateItemsService(items, itemToUpdate)).toEqual([itemToUpdate])
		})
	...
	describe('filterAndAddItemService', () => {
		...
		it('Returns items if items is empty', () => {
			const items = []
			expect(filterAndAddItemService(items, item)).toEqual(items)
		})

		it('Returns items if item is empty', () => {
			const item = []
			expect(filterAndAddItemService(items, item)).toEqual(items)
		})

		it('Returns filtered items new value', () => {
			const item = { _id: 'foo', value: 'bar' }

			const expected = [{ _id: 'bar', value: 'bar' }, item]
		...
		it('Returns sliced items', () => {
			const items = Array.from({ length: ANNOTATION_ITEMS_SIZE + 1 }, (v, i) => ({
				_id: `foo_${i}`,
				value: `foo_value_${i}`,
			}))
			expect(filterAndAddItemService(items, item)).toHaveLength(ANNOTATION_ITEMS_SIZE)
		})
		...
		it('Returns filtered and sliced items', () => {
			...
			const item = { _id: 'foo_1', value: 'foo_value_1' }
			expect(filterAndAddItemService(items, item)).toHaveLength(ANNOTATION_ITEMS_SIZE)
		})
	})
